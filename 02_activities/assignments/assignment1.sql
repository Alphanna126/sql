...
select distinct vendor_id, count(vendor_id) as booth_rent_count
...
strftime('%Y' , market_date ) as year  from customer_purchases;
...
SELECT customer_id , strftime('%m' , market_date ) as month , 
strftime('%Y' , market_date ) as year , SUM(quantity*cost_to_customer_per_qty) AS total  
from customer_purchases
